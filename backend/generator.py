...
def generate_answer(user_query, top_k=10,similarity_threshold=0.75):
    chunks = []
    keyword_hint = None
    ...
    try:
        ...
        before_filtering = len(chunks)
        chunks = [c for c in chunks if c.get("score", 1.0) >= similarity_threshold]
        print(f"🎯 {len(chunks)} chunks remained after applying similarity threshold ≥ {similarity_threshold}.")
