...
def find_enclosing_struct(node):
    while node:
...
def walk_and_extract(source_code, root):
    ...
    def walk(node):
        ...
        if ntype == "function_definition":
            ...
            parent_struct = find_enclosing_struct(node)
            hierarchy = f"{parent_struct}.{func_name}" if parent_struct else func_name
            chunks.append({
                "type": "function",
                "start_line": sl,
                "end_line": el,
            ...
            name_node = node.child_by_field_name("name")
            struct_name = name_node.text.decode("utf-8") if name_node else None
            chunks.append({
                "type": "struct",
                "name": struct_name,
                "hierarchy": struct_name,
                "start_line": sl,
                "end_line": el,
                "code": text
            })
