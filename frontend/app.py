...
def render_home():
    ...
    st.markdown(
        f"""
        <h1 style="font-size: 2.3rem; text-align: center; color: {accent_color}; margin-top: 0rem;">
    ...
    mode = st.radio("", ["📁 Manual Upload", "🗂 File Paths", "📂 Select from Codebase"], horizontal=True)
    ...
    else:  # 📂 Select from Codebase
        st.markdown(f"<div style='color: {text_color};'>Select C/C++ file(s) from existing codebase</div>", unsafe_allow_html=True)

        def list_code_files(directory, extensions=[".c", ".cpp", ".h", ".hpp"]):
            code_files = []
            ...
            if not code_files:
                ...
                if selected_files:
                    try:
                        ...
                        st.session_state.uploaded_file_names = [os.path.basename(f) for f in selected_files]
                    except Exception as e:
                        st.error(f"⚠ Error opening selected files: {e}")
    ...
    if st.session_state.upload_successful and st.session_state.uploaded_file_names:
        ...
        for name in st.session_state.uploaded_file_names:
            st.markdown(f"<div style='margin-bottom: 0.3rem; color: {text_color};'>• {name}</div>", unsafe_allow_html=True)
